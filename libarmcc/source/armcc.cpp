...
static uint32_t gen_arm_group_rd_rn_x(armcc_p const p2cc, const unsigned group,
	const arm_reg_t rd, const arm_reg_t rn, const uint32_t x)
{ return(_gen_arm_cc_group_ir(p2cc, CC_AL, group, gen_arm_rd_rn(rd, rn)	| x)); }

/* **** */

uint32_t armcc::add(const arm_reg_t rd, const arm_reg_t rn, const uint8_t imm)
{
	return(gen_arm_dp__op_s_rd_rn_sop(&cc, ARM_ADD, 0, rd, rn,
		gen_arm_dp_sop__ror_i(imm, 0)));
}
...
{ return(_gen(&cc, 5, arm::b::offset(&cc, pat))); }

uint32_t armcc::bne(const uint32_t pat)
{ return(gen_arm_cc_group_ir(&cc, CC_NE, 5, arm::b::offset(&cc, pat))); }

uint32_t armcc::cmp(const arm_reg_t rn, const arm_reg_t rm)
{ return(gen_arm_dp_op_s_rd_rn_rm(&cc, ARM_CMP, 1, r0, rn, rm)); }
...
{
	unsigned bit_u = 0;
	const uint32_t offset = arm::ldst::offset(&cc, pat, &bit_u);

	const uint32_t ir = (1 << ARM_IR_LDST_BIT_L)
		| (1 << ARM_IR_LDST_BIT_P)
		| (bit_u << ARM_IR_LDST_BIT_U);

	return(gen_arm_group_rd_rn_x(&cc, 2, rd, rn, ir + offset));
}

uint32_t armcc::mov(const arm_reg_t rd, const arm_reg_t rm)
{ return(gen_arm_dp_op_s_rd_rn_rm(&cc, ARM_MOV, 0, rd, r0, rm)); }

uint32_t armcc::mov(const arm_reg_t rd, const uint8_t imm)
{
	return(gen_arm_dp_op_s_rd_rn(&cc, 1, ARM_MOV, 0, rd, r0, gen_arm_dp_sop_ror_i(imm, 0)));
}
