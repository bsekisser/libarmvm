...
static
int __thumb_fail_decode(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_add_rd_pcsp_i(armvm_core_ref core)
{
	const int pcsp = bext32(IR, 11);

	uint32_t rn = 0;
	if(pcsp)
		rn = reg_src(core, rRN, rSP);
	else
		rn = reg_setup_rR_vR(core, rRN, rPC, THUMB_PC_NEXT & ~3U);

	const uint16_t imm8 = mlBFMOV(IR, 7, 0, 2);

	reg_dst_decode(core, rRD, 10, 8);
...
static
int _armvm_core_thumb_add_sub_rn_rd__rm(armvm_core_ref core,
...
{
	const uint32_t rm = reg_setup_vRml(core, rRM, 8, 6);
...
{
	const uint32_t rm = reg_src_decode(core, rRM, 8, 6);

	return(_armvm_core_thumb_add_sub_rn_rd__rm(core, 0, rm));
}

static
int _armvm_core_thumb_add_sub_sp_i7(armvm_core_ref core)
{
	const int sub = bext32(IR, 7);
	const uint16_t imm7 = mlBFMOV(IR, 6, 0, 2);

	const uint32_t rn = reg_src(core, rRN, rSP);
	...
	if(_itrace_start(core, 0)) {
		...
		_itrace_end_with_comment(core, "0x%08x %s0x%04x = 0x%08x",
...
static
int _armvm_core_thumb_bcc(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_bx(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_bxx__bl_blx(armvm_core_ref core,
...
static
int _armvm_core_thumb_bxx_b(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_bxx_bl(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_bxx_prefix(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_dp_rms_rdn(armvm_core_ref core)
{
...
static
int _armvm_core_thumb_ldst_rd_i(armvm_core_ref core)
{
	...
	switch(operation)
	{
		case	0x4000:
			...
			break;
		case	0x9000:
			rn = reg_setup_rR_vR(core, rRN, rSP, SP);
			break;
		default:
	...
	const uint32_t ea = reg_setup_vR(core, rREA, rn + imm8);
	int ldst_rval = 0;
	...
	if(_itrace_start(core, 0)) {
		...
		_itrace_end_with_comment(core, "[0x%08x](0x%08x)",
			ea, vR(D));
	}
...
static
int _armvm_core_thumb_ldst_bwh_o_rn_rd(armvm_core_ref core)
{
//	struct {
...
//	}bit;
	...
	reg_setup_rRml(core, rRD, 2, 0);

	const unsigned rm_shift = bit_h ? 1 : (bit_b ? 0 : 2);
	const uint32_t rm = reg_setup_vR(core, rRM, mlBFMOV(IR, 10, 6, rm_shift));

	const uint32_t ea = reg_setup_vR(core, rREA, rn + rm);
	...
	if(_itrace_start(core, 0)) {
		...
		_itrace_end_with_comment(core, "[(0x%08x + 0x%03x) = 0x%08x](0x%08x)",
			rn, rm, ea, vR(D));
	}
...
static
int _armvm_core_thumb_ldst_rm_rn_rd(armvm_core_ref core)
{
//	struct {
...
static
int _armvm_core_thumb_ldstm_rn_rxx(armvm_core_ref core)
{
//	struct {
		const int bit_l = bext32(IR, 11);
//	}bit;

	const uint32_t rn = reg_src_decode(core, rRN, 10, 8);
...
static
int _armvm_core_thumb_pop_push(armvm_core_ref core)
{
//	struct {
		const int bit_l = bext32(IR, 11);
...
//	}bit;
	...
	uint32_t ea = reg_setup_vR(core, rREA, start_address & ~3U);
...
int _armvm_core_thumb_sbi_imm5_rm_rd(armvm_core_ref core)
{
	...
	const uint32_t rs = reg_setup_vR(core, rRS, mlBFEXT(IR, 10, 6));
...
static
int armvm_core_thumb__step_group0_0000_1fff(armvm_core_ref core)
{
...
static
int armvm_core_thumb__step_group2_4000_5fff(armvm_core_ref core)
{
...
static
int armvm_core_thumb__step_group5_b000_bfff(armvm_core_ref core)
{
	switch(mlBFTST(IR, 15, 8)) {
...
static
int armvm_core_thumb__step_group6_c000_dfff(armvm_core_ref core)
{
	if(btst32(IR, 12)) {
		switch(mlBFTST(IR, 15, 8)) {
...
static
int armvm_core_thumb__step_group7_e000_ffff(armvm_core_ref core)
{
